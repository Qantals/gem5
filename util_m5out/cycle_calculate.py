...
def extract_values_from_stats(
    ...
    pattern_cpu_cycles = re.compile(
        r"^system\.cpu(\d+)\.numCycles\s+(\d+)\s+#.*$"
    )
    pattern_gpu_no_issue = re.compile(
        r"^system\.cpu(\d+)\.CUs(\d+)\.ExecStage\.numCyclesWithNoIssue\s+(\d+)\s+#.*$"
    )
    pattern_gpu_instr_issued = re.compile(
        r"^system\.cpu(\d+)\.CUs(\d+)\.ExecStage\.numCyclesWithInstrIssued\s+(\d+)\s+#.*$"
    )
        ...
        for line in f:
            if pattern_start.match(line):
                ...
                if match_gpu_instr_issued:
                    ...
                    num_cycles_instr_issued = int(
                        match_gpu_instr_issued.group(3)
                    )
                    if cpu_id not in current_stats:
                        current_stats[cpu_id] = {}
                    if cu_id not in current_stats[cpu_id]:
                    ...
                    ] = num_cycles_instr_issued

    return stats_list


def compute_max_cycles(
    stats_list: List[Dict[str, Dict[str, int]]],
) -> List[Dict[str, int]]:
    """
                ...
                if "numCycles" in v:
                    ...
                    for cu_id, cu_data in v.items():
                        if isinstance(cu_data, dict):  # Check if it's a GPU CU
                            num_cycles_no_issue = cu_data.get(
                                "numCyclesWithNoIssue", 0
                            )
                            num_cycles_instr_issued = cu_data.get(
                                "numCyclesWithInstrIssued", 0
                            )
                            total_gpu_cycles = (
                                num_cycles_no_issue + num_cycles_instr_issued
                            )
                            max_gpu_cycles = max(
                                max_gpu_cycles, total_gpu_cycles
                            )
