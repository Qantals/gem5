...
def extract_values_from_log(log_file_path: Path):
    ...
    pattern_freq = re.compile(
        r"^command line:.*\s--CPUClock (\d+\.?\d*)GHz --gpu-clock (\d+\.?\d*)GHz\s.*$"
    )
    ...
    result["latency_str"] = "".join(
        str(result[f"latency_{i}"]) for i in range(len(target_ids))
    )
...
def main():
    ...
    for result in results:
        ...
        new_folder_name = f"freq{result['freq_cpu']:.1f}_{result['freq_gpu']:.1f}lat{result['latency_str']}"
